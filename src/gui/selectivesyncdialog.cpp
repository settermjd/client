    ...
    , _folderTree(new QTreeWidget(this))
{
    ...
    header->setText(tr("Select remote subfolders you wish to synchronize."));
    ...
    const auto sizeLimit = ConfigFile().newBigFolderSizeLimit();
    if (sizeLimit.first) {
        // The size limit is stored in MB
        _bigFolderSizeLimitBytes = sizeLimit.second * 1024 * 1024;
    }
}
