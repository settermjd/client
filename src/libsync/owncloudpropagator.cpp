...
bool PropagateDirectory::scheduleNextJob()
{
    ...
    while (subJobsIterator.hasNext()) {
        ...
        if (subJobsIterator.value()->_state == Finished) {
            // If this items is finished, remove it from the _subJobs vector as it is not needed anymore
            // Note that in this case remove() from QVector will just perform memmove of pointer array items, most probably from vector head.
